...
disp('data may be any length, exactly 0 overlap regardless of data length')
...
fracfreqlim = .125;  % fractional frequency limit for b363
disp('using fracfreqlim 1/8, suitable for b363')
...
start_ind = linspace(1,length(dat)+1,L+1);
start_ind = start_ind(1:L);  % last entry is just for generation of sequence
...
disp(sprintf('working on image #%g of %g images',l,L))
  ...
  b = dat(ind:ind+length(dat)/L-1); % zero overlap regardless of data length
